...
def parse_ena_spreadsheet(request):
    ...
    ghlper.notify_read_status(
        data={"profile_id": profile_id}, msg='', action="info", html_id="sample_info"
    )
...
def save_ena_records(request):
    ...
    for line in range(1, len(sample_data)):
        ...
        if "biosampleAccession" in s:
                ...
                .find_one(
                    {
                        ...
                        "biosampleAccession": s["biosampleAccession"],
                    }
                )
            )
        else:
            ...
            sample = (
                Sample()
                .get_collection_handle()
                .find_one({"name": s["Sample"], "profile_id": profile_id})
            )
        ...
        if "Organism" in s:
            if not sample or sample.get("organism", "") != s["Organism"]:
                ...
                if not tax_id:
                    ...
                    termAccession = "http://purl.obolibrary.org/obo/NCBITaxon_" + str(
                        taxinfo[0]["taxId"]
                    )
                    source["organism"] = {
                        "annotationValue": s["Organism"],
                    ...
                    source_id = str(
                        ...
                        .find_one_and_update(
                            {
                                "organism.termAccession": termAccession,
                                "profile_id": profile_id,
                            },
        ...
        if "biosampleAccession" in s:
            ...
            if is_external_sample:
                sample_accession = {
                    "sample_accession": s["sraAccession"],
                    "biosample_accession": s["biosampleAccession"],
        ...
        if s["Library layout"] == "SINGLE":
            ...
            datafile = (
                DataFile()
                .get_collection_handle()
                .find_one({"file_location": file_location})
            )
            if datafile:
            ...
            result = (
                DataFile()
                .get_collection_handle()
                .update_one({"file_location": file_location}, {"$set": df}, upsert=True)
            )
        ...
        if not is_found:
            Sample(profile_id=profile_id).get_collection_handle().update_one(
                {"_id": ObjectId(sample_id)}, {"$addToSet": {"read": f_meta}}
            )
    ...
    if submission_external_sample_accession:
        Submission().get_collection_handle().update_one(
            ...
            {
                "$addToSet": {
                    "accessions.sample": {"$each": submission_external_sample_accession}
                }
            },
        )
    ...
    table_data = ena_read.generate_read_record(
...
def init_manifest_submission(request):
    ...
    if not doc:  # submission not in queue, add to queue
        ...
        context['message'] = (
            'Submission has been added to the processing queue. Status update will be provided.'
        )
    else:
        context['message'] = 'Submission is already in the processing queue.'

    ghlper.update_submission_status(
        status='info', message=context['message'], submission_id=submission_id
    )
...
def get_submission_status(request):
    ...
    for rec in submission_records:
        ...
        new_data['transcript_status'] = status.get(
            'type', str()
        )  # status type is: 'info', 'error', or 'success'
        ...
        new_data["complete"] = (
            "error" if new_data['transcript_status'] == 'error' else "pending"
        )
...
def get_read_accessions(request, sample_accession):
    samples = Sample().get_all_records_columns(
        ...
        projection={"profile_id": 1, "read": 1},
    )
    run_accessions = []
    experiment_accessions = []
    if samples:
        sample = samples[0]
        submission = Submission().get_all_records_columns(
            filter_by={"profile_id": sample["profile_id"]}, projection={"accessions": 1}
        )
        for read in sample.get("read", []):
                ...
                if read.get("status", "pending") == "accepted":
                    for accession in submission[0].get("accessions", {}).get("run", []):
                        if set(accession.get("datafiles", [])) == set(file_ids):
                            ...
                            break
                    for accession in (
                        submission[0].get("accessions", {}).get("experiment", [])
                    ):
                        if accession.get("alias", []) == alias:
                            experiment_accessions.append(
                                accession.get("accession", str())
                            )
                            break
    result = dict(
        run_accessions=run_accessions, experiment_accessions=experiment_accessions
    )
    return JsonResponse(status=200, data=result)
