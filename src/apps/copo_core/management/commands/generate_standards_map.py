...
class Command(BaseCommand):
    ...
    def load_data(self):
        ...
        # MIxS data
        self.mixs_mapping = {
            'COLLECTOR_SAMPLE_ID': {'mixs': 'samp_name'},
            'COMMON_NAME': {'mixs': 'host_common_name'},
            'CULTURE_OR_STRAIN_ID': {'mixs': 'microb_start_taxID'},
            'DATE_OF_COLLECTION': {'mixs': 'collection_date'},
            'DECIMAL_LATITUDE': {'mixs': 'lat_lon'},
            'DECIMAL_LONGITUDE': {'mixs': 'lat_lon'},
            'DEPTH': {'mixs': 'depth'},
            'DESCRIPTION_OF_COLLECTION_METHOD': {'mixs': 'samp_collect_method'},
            'DISSOLVED_OXYGEN': {'mixs': 'diss_oxygen'},
            'ELEVATION': {'mixs': 'elev'},
            'HABITAT': {'mixs': 'host_body_habitat'},
            'LIGHT_INTENSITY': {'mixs': 'light_intensity'},
            'ORIGINAL_GEOGRAPHIC_LOCATION': {'mixs': 'geo_loc_name'},
            'OTHER_INFORMATION': {'mixs': 'additional_info'},
            'PRESERVATION_APPROACH': {'mixs': 'samp_preserv'},
            'PRESERVATIVE_SOLUTION': {'mixs': 'samp_store_sol'},
            'PH': {'mixs': 'ph'},
            'PURPOSE_OF_SPECIMEN': {'mixs': 'samp_purpose'},
            'RELATIONSHIP': {'mixs': 'biotic_relationship'},
            'SCIENTIFIC_NAME': {'mixs': 'specific_host'},
            'SIZE_OF_TISSUE_IN_TUBE': {'mixs': 'size_frac'},
            'SPECIMEN_ID': {'mixs': 'source_mat_id'},
            'SEX': {'mixs': 'urobiom_sex'},
            'SYMBIONT': {'mixs': 'host_symbiont'},
            'TAXON_ID': {'mixs': 'samp_taxon_id'},
            'TIME_ELAPSED_FROM_COLLECTION_TO_PRESERVATION': {'mixs': 'timepoint'},
            'TEMPERATURE': {'mixs': 'samp_store_temp'},
            'TUBE_OR_WELL_ID': {'mixs': 'samp_well_name'},
            'tol_project': {'mixs': 'project_name'},
        }

        # DWC data
        self.DWC_FIELD_NAMES_MAPPING = {
            'DESCRIPTION_OF_COLLECTION_METHOD': {'dwc': 'samplingProtocol'},
            'DIFFICULT_OR_HIGH_PRIORITY_SAMPLE': {'dwc': 'samplingEffort'},
            'COLLECTION_LOCATION': {'dwc': 'higherGeography'},
            'COLLECTED_BY': {'dwc': 'recordedBy'},
            'COLLECTOR_AFFILIATION': {'dwc': 'institutionCode'},
            'COLLECTOR_ORCID_ID': {'dwc': 'recordedByID'},
            'COLLECTOR_SAMPLE_ID': {'dwc': 'materialSampleID'},
            'COMMON_NAME': {'dwc': 'vernacularName'},
            'CULTURE_OR_STRAIN_ID': {'dwc': 'occurrenceID'},
            'DATE_OF_COLLECTION': {'dwc': 'eventDate'},
            'DEPTH': {'dwc': 'verbatimDepth'},
            'ELEVATION': {'dwc': 'verbatimElevation'},
            'ETHICS_PERMITS_DEF': {'dwc': 'license'},
            'FAMILY': {'dwc': 'family'},
            'GAL_SAMPLE_ID:': {'dwc': 'institutionID'},
            'GENUS': {'dwc': 'genus'},
            'GRID_REFERENCE': {'dwc': 'georeferenceRemarks'},
            'HABITAT': {'dwc': 'locationRemarks'},
            'IDENTIFIED_BY': {'dwc': 'identifiedBy'},
            'ORDER_OR_GROUP': {'dwc': 'order'},
            'ORGANISM_PART': {'dwc': 'organismScope'},
            'ORIGINAL_COLLECTION_DATE': {'dwc': 'eventDate'},
            'PRESERVATION_APPROACH': {'dwc': 'preparations'},
            'RELATIONSHIP': {'dwc': 'relationshipRemarks'},
            'SAMPLE_COORDINATOR_AFFILIATION': {'dwc': 'institutionCode'},
            'SCIENTIFIC_NAME': {'dwc': 'scientificName'},
            'SERIES': {'dwc': 'fieldNumber'},
            'SIZE_OF_TISSUE_IN_TUBE': {'dwc': 'sampleSizeValue'},
            'SPECIMEN_ID': {'dwc': 'materialSampleID'},
            'TIME_OF_COLLECTION': {'dwc': 'eventTime'},
        }

        # Sample attributes json schema
        sample_json_schema = self.read_json_file(lk.WIZARD_FILES['sample_details'])
        self.sample_attributes_file_paths = [
            os.path.join(settings.SCHEMA_VERSIONS_DIR, prop['$ref'])
            for prop in sample_json_schema.get('properties', [])
            if '$ref' in prop
        ]
    ...
    def generate_json_file(
        ...
        file_name,
        data,
        directory_path=os.path.join(settings.SCHEMA_VERSIONS_DIR, 'isa_mappings'),
    ):
        ...
        with open(file_path, 'w+') as f:
            f.write(
                json.dumps(
                    data, indent=4, sort_keys=False, default=str, ensure_ascii=False
                )
            )

        self.stdout.write(
            self.style.SUCCESS(f'Successfully generated the file: {file_name}')
        )
    ...
    def get_tol_project_defined_fields(self):
        ...
        tol_fields = [
            x['id'].split('.')[-1]
            for x in schema
            if x.get('specifications', list()) and 'tol' in x.get('standards', dict())
        ]
    ...
    def fetch_partial_mixs_data_from_excel(self):
        ...
        terms_version = 'v6'
        url = f'https://github.com/GenomicsStandardsConsortium/mixs/raw/refs/heads/main/release/excel/mixs_{terms_version}.xlsx'
        ...
        if response.status_code == 200:
            ...
            if not all(col in excel_data.columns for col in columns_to_extract):
                print(
                    f'Missing required columns: {set(columns_to_extract) - set(excel_data.columns)}'
                )
                return list()
            ...
            for _, row in excel_data.iterrows():
                ...
                if (
                    pd.notnull(structured_comment_name)
                    and pd.notnull(definition)
                    and pd.notnull(mixs_id)
                ):
            ...
            for key, value_dict in mixs_schema.items():
                ...
                def add_data(field):
                    ...
                    if unique_identifier not in added_fields:
                        ...
                        if data:
                            description = (
                                data[field].get('description', '')
                                if data
                                else value_dict.get('description', '')
                            )
                            uri = (
                                data[field].get('uri', '')
                                if data
                                else value_dict.get('uri', '')
                            )
                        else:
    ...
    def create_tol_mixs_mapping(self):
        ...
        for tol_field in self.tol_fields_lst:
            ...
            if tol_field in self.mixs_mapping:
                ...
                mixs_field_info = next(
                    (x for x in self.mixs_fields if x.get('field', '') == mixs_field),
                    None,
                )
            ...
            mixs_field_info = next(
                (
                    x
                    for x in self.mixs_fields
                    if tol_field.lower().replace('_', '') in x['field'].lower()
                    or tol_field.lower() in x['field'].lower()
                ),
                None,
            )
        ...
        # print(f'\out:\n {json.dumps(out, indent=4, sort_keys=False,default=str, ensure_ascii=False)}\n')
    ...
    def generate_dwc_fields_json(self):
        ...
        try:
            ...
            df_filtered = df[
                (df['issued'] == df_latest_term_versions)
                & (df['status'] == 'recommended')
            ]

            # Convert the filtered dataframe to JSON format
            dwc_json_latest_non_deprecated_term_versions = df_filtered[
                ['field', 'description', 'uri']
            ...
            # print(json.dumps(dwc_json, indent=4, sort_keys=False, default=str, ensure_ascii=False))
    ...
    def create_tol_dwc_mapping(self):
        ...
        for tol_field in self.tol_fields_lst:
            ...
            if tol_field in self.DWC_FIELD_NAMES_MAPPING:
                ...
                dwc_field_info = [
                    dwc_field_dict
                    for dwc_field_dict in self.dwc_fields
                    if dwc_field_dict.get('field', '') == dwc_field
                ]
            else:
                ...
                dwc_field_info = [
                    dwc_field_dict
                    for dwc_field_dict in self.dwc_fields
                    if tol_field.lower().replace('_', '')
                    in dwc_field_dict['field'].lower()
                    or tol_field.lower() in dwc_field_dict['field'].lower()
                ]
    ...
    def generate_standards_map(self):
        ...
        for tol_field in self.tol_fields_lst:
            ...
            if tol_field in self.ena_field_names_mapping:
                ena['field'] = self.ena_field_names_mapping.get(tol_field, str()).get(
                    'ena', str()
                )
                ...
                if (
                    tol_field in self.ena_and_tol_rules
                    and 'ena_regex' in self.ena_and_tol_rules[tol_field]
                ):
                    ena['regex'] = self.ena_and_tol_rules[tol_field]['ena_regex']

                if (
                    tol_field in self.ena_and_tol_rules
                    and 'human_readable' in self.ena_and_tol_rules[tol_field]
                ):
                    ena['description'] = self.ena_and_tol_rules[tol_field][
                        'human_readable'
                    ]
            ...
            if tol_field in self.ena_and_tol_rules and 'strict_regex' in list(
                self.ena_and_tol_rules[tol_field].keys()
            ):
                ena['regex'] = self.ena_and_tol_rules[tol_field]['strict_regex']

            if tol_field in self.ena_and_tol_rules and 'human_readable' in list(
                self.ena_and_tol_rules[tol_field].keys()
            ):
        ...
        # print(f'\nStandards_map:\n {json.dumps(output, indent=4, sort_keys=False, default=str, ensure_ascii=False)}\n')
        ...
        for schema_file_path in self.sample_attributes_file_paths:
                ...
                if key in standards_map_data:
                    ...
                    for standard in entry['standards']:
                        if standard in standards_map_data[key]:
                            new_value = standards_map_data[key][standard].get(
                                'field', ''
                            )
            ...
            if updated:
                self.generate_json_file(
                    file_name=file_name, data=schema_data, directory_path=directory_path
                )
