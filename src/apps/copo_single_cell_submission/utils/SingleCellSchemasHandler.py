...
class SingleCellSchemasHandler:
    ...
    def _parseSchemas(self, schema_name, xls):
        ...
        schemas_df["regex_valid"] = schemas_df["term_regex"].apply(
            lambda x: self._validate_regrex(x)
        )
        ...
        for i, j in zip(*np.where(pd.isnull(schemas_df[column_names]))):
            empty_values = (
                empty_values + f"Empty value in '{column_names[j]}' at row {i+2} \n"
            )
        if empty_values:
            raise Exception(
                f'Empty values: {empty_values} in the schema. Please check the schema file.'
            )
        ...
        for checklist_id in checklist_df.index:
            ...
            for c in (
                checklist_schema_df.groupby(["component_name", "term_name"])
                .size()
                .reset_index()
                .values
            ):
                if c[2] > 1:
                    raise Exception(
                        f'Duplicate term_name: "{c[1]}" within a component: "{c[0]}"  with same version: {checklist_id}.'
                    )

            for c in (
                checklist_schema_df.groupby(["component_name", "term_label"])
                .size()
                .reset_index()
                .values
            ):
                if c[2] > 1:
                    raise Exception(
                        f'Duplicate item_label: "{c[1]}" within a component: "{c[0]}"  with same version: {checklist_id}.'
                    )

            # check foreign key constraints
            identifier_map = {}
            referenced_df = checklist_schema_df.drop(
                checklist_schema_df[
                    pd.isna(checklist_schema_df["referenced_component"])
                ].index
            )
            ...
            for c, row in referenced_df.iterrows():
                if (
                    row["referenced_component"]
                    and row["referenced_component"] not in identifier_map.keys()
                ):
                    raise Exception(
                        f"Referenced component: '{row['referenced_component']}' is missing"
                    )
            ...
            if format == "xlsx":
                with pd.ExcelWriter(path=file_path, engine='xlsxwriter') as writer:
                    ...
                    for component_name, schema in schemas.items():
                        ...
                        component_schema_df.loc[
                            component_schema_df["mandatory"] == "M", "term_label"
                        ] = component_schema_df["term_label"]
                        component_schema_df.loc[
                            component_schema_df["mandatory"] != "M", "term_label"
                        ] = (component_schema_df["term_label"] + " (optional)")
                        component_schema_df["empty_column"] = ""

                        component_schema_df_transposed = component_schema_df.transpose()
                        component_schema_df_transposed = (
                            component_schema_df_transposed.loc[
                                [
                                    "term_label",
                                    "term_description",
                                    "term_example",
                        ...
                        component_schema_df_transposed.columns = (
                            component_schema_df_transposed.iloc[0]
                        )
                        ...
                        if singlecell is not None:
                            ...
                            if not component_data_df.empty:
                                new_column_name = {
                                    field["term_name"]: field["term_label"]
                                    + (
                                        " (optional)"
                                    ...
                                    for field in schema
                                    if field[schema_checklist] in ["M", "O"]
                                }
                                component_data_df.drop(
                                    columns=list(
                                        set(component_data_df.columns)
                                        - set(new_column_name.keys())
                                    ),
                                    inplace=True,
                                )
                                ...
                                component_schema_df_transposed = pd.concat(
                        ...
                        for index, field in component_schema_df.iterrows():
                            ...
                            if name not in component_schema_df_transposed.columns:
                                continue
                            column_index = (
                                component_schema_df_transposed.columns.get_loc(name)
                            )
                            ...
                            if field["identifier"]:
                                ...
                                if prefix:
                                    ...
                                    if column_letter != 'A':
                                        ...
                                        writer.sheets[sheet_name].write_array_formula(
                                            f'{column_letter}{starting_number}:{column_letter}100',
                                            f'{{=IF(ISBLANK(C{starting_number}:C100)*ISBLANK(D{starting_number}:D100)*ISBLANK(E{starting_number}:E100)*ISBLANK(F{starting_number}:F100), "" , CONCATENATE("{prefix}", TEXT(ROW()-4,"000")))}}',
                                            cell_format,
                                        )
                            ...
                            if index == "study_id" and sheet_name != "study":
                                ...
                                writer.sheets[sheet_name].write_array_formula(
                                    f'{column_letter}{starting_number}:{column_letter}100',
                                    f'{{=IF(ISBLANK(C{starting_number}:C100)*ISBLANK(D{starting_number}:D100)*ISBLANK(E{starting_number}:E100)*ISBLANK(F{starting_number}:F100), "", IF(study!$A$5 <> "", study!$A$5, "" ))}}',
                                    cell_format,
                                )
                                # writer.sheets[sheet_name].write(f"{column_letter}1", name, cell_format)
                                ...
                                referenced_schema_df = pd.DataFrame.from_records(
                                    referenced_schema
                                )
                                referenced_schema_df = referenced_schema_df.drop(
                                    referenced_schema_df[
                                        pd.isna(referenced_schema_df[schema_checklist])
                                    ].index
                                )
                                referenced_schema_df.reset_index(inplace=True)
                                if not referenced_schema_df.empty:
                                    referenced_field = referenced_schema_df.loc[
                                        referenced_schema_df['identifier']
                                    ]
                                ...
                                if len(choice) > 0:
                                    source = ""
                                    number_of_char_for_choice = sum(
                                        [len(str(x)) for x in choice]
                                    )
                        ...
                        if not component_data_df.empty:
                            context.append(
                                request.build_absolute_uri(
                                    f"/media/assets/manifests/{schema_name}_{checklist}_{component_name}{version}.jsonld"
                                )
                            )
                            new_column_name = {
                                field["term_name"]: (
                                    ...
                                    + reverse(
                                        ...
                                        args=[
                                            singlecell_schema["name"],
                                            field["term_name"],
                                        ],
                                    )
                                    if not field["term_reference"]
                                    or not isinstance(field["term_reference"], str)
                                    or field["term_type"] == "ontology"
                                    else field["term_reference"]
                                )
                                for field in schema
                                if field[schema_checklist] in ["M", "O"]
                            }
    ...
    def generate_jsonld(self, singlecell_schema, checklist_id=None):
        ...
        for id, checklist in checklists.items():
            ...
            for component_name, schema in schemas.items():
                component_schema_df = pd.DataFrame.from_records(schema)
                component_schema_df = component_schema_df.drop(
                    component_schema_df[pd.isna(component_schema_df[id])].index
                )
                if component_schema_df.empty:
                    continue
                component_schema_df = component_schema_df.astype(object).fillna("")
                ...
                for _, row in component_schema_df.iterrows():
                    ...
                    if term_reference and term_type != "ontology":
                        ...
                        term_reference = url_prefix + reverse(
                            "copo_single_cell_submission:display_term",
                            args=[singlecell_schema["name"], term_name],
                        )
                    else:
                        continue
                ...
                try:
                    ...
                    l.error(
                        f"Failed to generate JSON-LD context for {schema_name} {id} {component_name}: {str(e)}"
                    )
                    continue
...
class SinglecellschemasSpreadsheet:
    def __init__(
        self, file, profile_id, schema_name, checklist_id, component, validators=[]
    ):
        ...
        for element_name in dir(required):
            ...
            if (
                ...
                and issubclass(element, Validator)
    ...
    def get_filenames_from_manifest(self):
        ...
        for component, df in self.new_data.items():
            ...
            schema_file_df = schema_df.loc[
                schema_df['term_type'] == 'file', ["term_name"]
            ]
            if schema_file_df.empty:
                continue

            schema_file_checksum_df = schema_df.loc[
                schema_df['term_name'].isin(
                    [
                        name + "_checksum"
                        for name in schema_file_df["term_name"].tolist()
                    ]
                ),
                ["term_name"],
            ]
            if not schema_file_checksum_df.empty:
                schema_file_checksum_df["term_checksum"] = schema_file_checksum_df[
                    "term_name"
                ]
                schema_file_checksum_df["term_name"] = schema_file_checksum_df[
                    "term_name"
                ].str.replace("_checksum", "", regex=False)
                    ...
                    if file_name:
                        if row["term_checksum"]:
                            if row2[row["term_checksum"]].strip():
                                file_map[row2[row["term_name"]]] = row2[
                                    row["term_checksum"]
                                ]
                            else:
                                msg = (
                                    ...
                                    + "File name: "
                                    + file_name
                                    + f' is missing checksum <b>{row["term_checksum"]}</b> in the manifest.<br/>'
                                )
        ...
        duplicated_files = [
            item for item, count in collections.Counter(filelist).items() if count > 1
        ]
        if duplicated_files:
            msg = (
                msg
            ...
            try:
                ...
                if m_format == "xls":
                    self.data = pd.read_excel(
                        ...
                        dtype=str,
                        converters={
                            0: lambda x: "" if x == 0 else x,
                            1: lambda x: "" if x == 0 else x,
                        },
                    )
                ...
                if self.schemas:
                    ...
                    for component, df in self.data.items():
                        ...
                        new_column_name = {
                            item["term_label"]: key
                            for key, item in self.schemas[component].items()
                        }
        ...
        for component, df in self.new_data.items():
            for column in df.columns:
                if column.startswith(Validator.PREFIX_4_NEW_FIELD):
                    self.data[column.removeprefix(Validator.PREFIX_4_NEW_FIELD)] = (
                        self.new_data[column]
                    )

        # if we get here we have a valid spreadsheet
        # notify_singlecell_status(data={"profile_id": self.profile_id}, msg="Spreadsheet is valid. Please click <b>Finish</b> to complete the upload.",
    ...
    def collect(self):
        ...
        notify_singlecell_status(
            data={"profile_id": self.profile_id, "components": list(self.data.keys())},
            msg=singlecell_data,
            action="make_table",
            html_id=f"{self.component}_parse_table",
            checklist_id=self.checklist_id,
        )
