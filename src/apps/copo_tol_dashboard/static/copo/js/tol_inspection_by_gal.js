...
$(document).ready(function () {
  ...
  $(document).on('click', '.taxonomyLevel_fieldName', function (el) {
    let previous_active_taxonomy_level = $(
      '#taxonomyLevelsDivID > input.active_taxonomy_level'
    );
    let taxonomy_field_name =
      el.target === undefined || el.target === null
        ? previous_active_taxonomy_level.val()
        : el.target.value;
    // Remove previous active taxonomy level
    previous_active_taxonomy_level.css({ 'background-color': '' });

    $('#taxonomyLevelsDivID input.active_taxonomy_level').removeClass(
      'active_taxonomy_level'
    );
  ...
  $('.tablinks').click(function () {
    ...
    !!this.outerHTML.includes(tablink2_ID)
...
function populate_pie_chart(el) {
  let gal_name_row =
    $(document).data('selected_gal_name_row') ||
    $('#gal_names').DataTable().rows('.selected').nodes()[0];
  ...
  $.ajax({
    ...
    .done(function (data) {
      ...
      if (data.length) {
        ...
        included_fields_tol_inspect_gal = [
          'ORDER_OR_GROUP',
          'FAMILY',
          'GENUS',
          'SCIENTIFIC_NAME',
        ...
        $(data).each(function (idx, db_data) {
          for (let db_field_name in db_data) {
            if (
              taxonomy_field_name === db_field_name &&
              included_fields_tol_inspect_gal.includes(db_field_name)
            ) {
          ...
          const field_name_occurences = pie_chart_labels.reduce(
          ...
          pie_chart_background_colours = Array.apply(
            null,
            Array(pie_chart_labels_distinct.length)
          ).map(function () {
        ...
        if (Chart.getChart('pieChartID') !== undefined)
          Chart.getChart('pieChartID').destroy();
                ...
                tooltip: {
                  callbacks: {
                    label: ({ label, formattedValue }) => [
                      `\xa0\xa0Name of ${taxonomy_field_name}: ${label}`,
                      `\xa0\xa0Number of sample associations: ${formattedValue}`,
                    ],
                  },
                },
              },
        ...
        if (window.location.href.endsWith('tol_dashboard/tol')) {
          ...
          if (current_gal_name)
            $('#current_gal_name').text(`GAL: ${current_gal_name}`);
        } else {
          component_loader
            .find('.piechart_spinner')
            .toggleClass('active')
...
function populate_bar_graph() {
  ...
  bar_graph_background_colours = bar_graph_border_colours.map((colour) =>
    colour.replace(')', ', 0.2)').replace('rgb', 'rgba')
  );

  // Generate a random number between 1 and 100
  gal_sample_goal_percentage_lst = Array.apply(
    null,
    Array(gal_names_lst.length)
  ).map(function () {
    return Math.floor(Math.random() * 100 + 1);
  });
  // Check if there is an existing instance of bar graph, if there is, destroy it
  if (Chart.getChart('barGraphID') !== undefined)
    Chart.getChart('barGraphID').destroy();
...
function get_gal_names() {
  ...
  $.ajax({
    url: '/copo/tol_dashboard/get_gal_names',
    method: 'GET',
    dataType: 'json',
    data: {},
  })
    .done(function (data) {
      if (data.length == 0) {
        ...
        if (window.location.href.endsWith('tol_dashboard/tol')) {
          ...
          piechart_component_loader
            ...
            .toggleClass('hidden');
          $(
            '<div class="emptyPiechartInfo">Details unavailable</div>'
          ).insertAfter(piechart_component_loader);
      ...
      $(data).each(function (d) {
        gal_names_table
          .find('tbody')
          .append(
            "<tr class='gal_name_selectable_row'><td style='max-width: 10px; text-align: center'>" +
              data[d] +
              '</td></tr>'
          );
      });
