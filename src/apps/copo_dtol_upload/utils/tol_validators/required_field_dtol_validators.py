...
class ColumnValidator(Validator):
    def validate(self):
        ...
        notify_frontend(
            data={"profile_id": self.profile_id},
            msg="Validating columns: Finished",
            action="info",
        ...
        for header, cells in self.data.items():
            ...
            if header in self.fields:
                if header == "SYMBIONT" and p_type in ["ASG", "DTOL", "ERGA"]:
                    ...
                    header in POP_GENOMICS_OPTIONAL_COLUMNS_DEFAULT_VALUES_MAPPING
                    and "ERGA" == p_type
                    and "POP_GENOMICS" in associated_p_type_lst
                    and len(associated_p_type_lst) == 1
                ):
                    # ERGA manifests that include "POP_GENOMICS" as an associated tol project type
                    ...
                    for c in cells:
                        ...
                        if not c.strip():
                            ...
                            if header == "SYMBIONT":
                                self.errors.append(
                                    msg["validation_msg_missing_symbiont"]
                                    % (
                                        header,
                                        str(cellcount + 1),
                                        "TARGET and SYMBIONT",
                                ...
                                self.errors.append(
                                    msg["validation_msg_missing_data"]
...
class RackTubeNotNullValidator(Validator):
    def validate(self):
        for index, row in self.data.iterrows():
            if (
                row.get("RACK_OR_PLATE_ID", "") in BLANK_VALS
                and row["TUBE_OR_WELL_ID"] in BLANK_VALS
            ):
                self.errors.append(
            ...
            if (
                row.get("RACK_OR_PLATE_ID", "") in BLANK_VALS
                or row["TUBE_OR_WELL_ID"] in BLANK_VALS
            ):
                self.errors.append(
            ...
            if (
                row.get("RACK_OR_PLATE_ID", "") in NA_VALS
                or row["TUBE_OR_WELL_ID"] in NA_VALS
            ):
                self.errors.append(
                    msg["validation_msg_rack_or_tube_is_na"] % (str(index + 1))
                )
...
class OrphanedSymbiontValidator(Validator):
    def validate(self):
        ...
        if not syms.empty:
            ...
            for el in list(tid):
                target = self.data.loc[
                    (self.data["SYMBIONT"] == "TARGET")
...
class RackPlateUniquenessValidator(Validator):
    def validate(self):
        ...
        if any(
            slash in value
            for value in list(self.data.get("RACK_OR_PLATE_ID", ""))
            for slash in SLASHES_LIST
        ) or any(
            slash in value
            for value in list(self.data.get("TUBE_OR_WELL_ID", ""))
            for slash in SLASHES_LIST
        ):
            self.errors.append(msg["validation_msg_rack_or_tube_contains_a_slash"])
            self.flag = False
        else:
            rack_tube = (
                self.data.get("RACK_OR_PLATE_ID", "")
                + "/"
                + self.data["TUBE_OR_WELL_ID"]
            )
            ...
            if len(dup) > 0:
                ...
                err = list(
                    map(
                        lambda x: x.get("RACK_OR_PLATE_ID", "")
                        + "/"
                        + x["TUBE_OR_WELL_ID"],
                        dup,
                    )
                ...
                existingsam = Sample().get_all_records_columns(
                    filter_by={"rack_tube": {"$in": err}},
                    projection={
                        "profile_id": 1,
                        "rack_tube": 1,
                ...
                for exsam in existingsam:
                    if exsam["profile_id"] == self.profile_id:
                        ...
                        self.errors.append(
                            msg["validation_msg_duplicate_tube_or_well_id_in_copo"]
                            % exsam["rack_tube"]
                        )
                        self.flag = False
            ...
            for i in u:
                rack, tube = i.split('/')
                rows = self.data.loc[
                    (self.data.get("RACK_OR_PLATE_ID", "") == rack)
                    & (self.data["TUBE_OR_WELL_ID"] == tube)
                ]
                counts = Counter([x.upper() for x in list(rows["SYMBIONT"].values)])
                if "TARGET" not in counts:
                    self.errors.append(
                        msg["validation_msg_duplicate_without_target"]
                        % (
                            str(
                                rows.get("RACK_OR_PLATE_ID", "")
                                + "/"
                                + rows["TUBE_OR_WELL_ID"]
                            )
                        )
                    )
                    self.flag = False
                ...
                if counts["TARGET"] + counts["SYMBIONT"] < len(
                    list(rows["SYMBIONT"].values)
                ):
                    self.errors.append(
                        msg["validation_msg_multiple_targets_with_same_id"] % (i)
                    )
...
class DecimalLatitudeLongitudeValidator(Validator):
    def validate(self):
        ...
        if "ERGA" == p_type:
            for index, row in self.data.iterrows():
                decimal_latlong_lst = [
                    row.get("DECIMAL_LATITUDE", ""),
                    row.get("DECIMAL_LONGITUDE", ""),
                ]
                latlong_start_end_lst = [
                    row.get("LATITUDE_START", ""),
                    row.get("LATITUDE_END", ""),
                    row.get("LONGITUDE_START", ""),
                    row.get("LONGITUDE_END", ""),
                ]

                if all(i == "" for i in decimal_latlong_lst) and all(
                    i == "" for i in latlong_start_end_lst
                ):
                    self.errors.append(
                        msg[
                            "validation_msg_error_decimal_latlong_or_latlong_start_end_missing_value"
                        ]
                        % str(index + 2)
                    )
                    self.flag = False
                elif any(i == "" for i in decimal_latlong_lst) or any(
                    i == "" for i in latlong_start_end_lst
                ):
                    # ERGA manifests that include "POP_GENOMICS" as an associated tol project type
                    # are allowed to have blank field
                    if (
                        any(i == "" for i in latlong_start_end_lst)
                        and "POP_GENOMICS" in associated_p_type_lst
                    ):
                        pass
                    else:
                    ...
                    any(i == "NOT_COLLECTED" for i in decimal_latlong_lst)
                    and any(i != "NOT_COLLECTED" for i in decimal_latlong_lst)
                    or any(i == "NOT_COLLECTED" for i in latlong_start_end_lst)
                    and any(i != "NOT_COLLECTED" for i in latlong_start_end_lst)
                ):
                    self.errors.append(
                        msg[
                            "validation_msg_error_decimal_latlong_or_latlong_start_end_mixed_value"
                        ]
                        % str(index + 2)
                    )
                    self.flag = False
                    ...
                    self.errors.append(
                        msg[
                            "validation_msg_error_decimal_latlong_or_latlong_start_end_all_not_collected"
                        ]
                        % str(index + 2)
                    )
                    self.flag = False
                    ...
                    self.errors.append(
                        msg[
                            "validation_msg_error_decimal_latlong_or_latlong_start_end_missing_start_end"
                        ]
                        % str(index + 2)
                    )
                    self.flag = False
                    ...
                    self.errors.append(
                        msg[
                            "validation_msg_error_decimal_latlong_or_latlong_start_end_missing_decimal_latlong"
                        ]
                        % str(index + 2)
                    )
                    self.flag = False
                    ...
                    self.errors.append(
                        msg[
                            "validation_msg_error_decimal_latlong_or_latlong_start_end_not_collected_mixed_value"
                        ]
                        % str(index + 2)
                    )
                    self.flag = False
                    ...
                    self.errors.append(
                        msg[
                            "validation_msg_error_decimal_latlong_or_latlong_start_end_all_contains_a_value"
                        ]
                        % str(index + 2)
                    )
                    self.flag = False
...
class PopGenomicsAssociatedTypeValidator(Validator):
    def validate(self):
        '''
        - Check if ERGA manifest is associated with
          Population Genomics (POP_GENOMICS) tol project type
        - Perform validation if this is the case
        - Set default values for the optional fields if they are empty/blank
          i.e. not filled in by manifest submitter
        '''
        profile = Profile().get_record(self.profile_id)
        p_type = profile.get('type', '').upper()
        associated_p_type_lst = profile.get('associated_type', [])
        p_name = profile.get('title', '')
            ...
            if (
                ...
                if (self.data['PURPOSE_OF_SPECIMEN'] == 'RESEQUENCING').all():
                    ...
                    for index, row in self.data.iterrows():
                        ...
                        for (
                            optional_column
                        ) in POP_GENOMICS_OPTIONAL_COLUMNS_DEFAULT_VALUES_MAPPING:
                            if not row.get(optional_column, '').strip():
                                if optional_column == 'GAL_SAMPLE_ID':
                                    default_value = self.data.at[
                                        index, 'COLLECTOR_SAMPLE_ID'
                                    ]
                                ...
                                self.warnings.append(
                                    msg['validation_msg_missing_optional_field_value']
                                    % (optional_column, str(index + 2), default_value)
                                )
                else:
                    # Display error message for rows where value of
                    # 'PURPOSE_OF_SPECIMEN' column is not equal to 'RESEQUENCING'
                    rows_indices = self.data.index[
                        self.data.get('PURPOSE_OF_SPECIMEN', '') != 'RESEQUENCING'
                    ].tolist()
                    ...
                    for index in rows_indices:
                        self.errors.append(
                            ...
                            % (
                                self.data.at[index, 'PURPOSE_OF_SPECIMEN'],
                                str(index + 2),
                            )
                        )
                        self.flag = False
                ...
                # Insert error message at the beginning of the list
                self.errors.insert(
                    0, msg['validation_msg_invalid_associated_tol_project'] % p_name
                )
                self.flag = False
            elif (
                'POP_GENOMICS' in associated_p_type_lst
