...
def copo_manifests(request):
    context = {
        'manifest_rows': [
            ...
            {
                'prefix': 'rembiImages',
                'label_prefix': 'General',
                'label_suffix': 'images',
                'title': 'Images based on REcommended Metadata for Biological Images (REMBI) standard',
                'is_header': True,
            },
            {
...
def build_manifest_item(
    ...
    return {
        ...
        'name': name,
        'label': label,
        'description': getattr(x, 'description', '') or None,
...
def populate_manifest_table(request):
    ...
    # COPO samples
    copo_sample_checklists = checklist_service.get_general_sample_checklists_no_fields()
    copo_sample_items = [
        build_manifest_item(
            x,
            manifest_id=x['primary_id'],
    ...
    copo_sample_items.sort(key=lambda x: x['label'])
    manifests.extend(copo_sample_items)
    ...
    ena_sample_items = [
        build_manifest_item(
            ...
            label=f"{x['primary_id']}: {(x.get('name') or '')}",
            blank_manifest_url=get_blank_manifest_url(x['primary_id']),
        )
        for x in copo_sample_checklists
        if not x['primary_id'].startswith('COPO')
    ]
    ena_sample_items.sort(key=lambda x: x['manifest_id'])
    manifests.extend(ena_sample_items)
    ...
    barcode_items.sort(key=lambda x: x['manifest_id'])
    manifests.extend(barcode_items)

    # COPO read
    schema_name = 'COPO_READ'
    copo_read_checklists = singlecell_service.get_checklists(
    ...
    copo_read_items = [
        build_manifest_item(
            ...
            manifest_type='copo_read',
            label=value.get('name', ''),
            blank_manifest_url=get_blank_manifest_url(schema_name, checklist_id),
            version=settings.MANIFEST_VERSION.get(schema_name, ''),
            is_single_cell=True,
        )
        for checklist_id, value in copo_read_checklists.items()
    ]
    copo_read_items.sort(key=lambda x: x['label'])
    manifests.extend(copo_read_items)

    # ENA read
    ena_read_checklists = checklist_service.get_sample_checklists_no_fields()
    ena_read_items = [
        build_manifest_item(
            x,
            manifest_id=x['primary_id'],
        ...
        for x in ena_read_checklists
        if x['primary_id'] == 'read'
    ]
    manifests.extend(ena_read_items)
    ...
    for profile in profile_service.get_profile_types():
        if profile.is_dtol_profile and profile.type not in excluded:
            ...
            item = build_manifest_item(
                profile,
                manifest_id=None,
                manifest_type='tol_samples',
                label=profile.description,
                show_prefill_wizard=True,
                blank_manifest_url=get_blank_manifest_url(profile_type),
                sop_url=get_sop_url(profile_type),
                technology=None,
                version=settings.MANIFEST_VERSION.get(profile_type, ''),
            )
            item.update({'name': profile_type, 'description': None})
            tol_items.append(item)
    manifests.extend(tol_items)

    # REMBI images
    schema_name = 'COPO_IMAGE_REMBI'
    rembi_checklists = singlecell_service.get_checklists(
        schema_name=schema_name, checklist_id=''
    )
    rembi_items = [
        build_manifest_item(
            value,
            manifest_id=checklist_id,
            manifest_type='rembi_images',
            label=value.get('name', ''),
            blank_manifest_url=get_blank_manifest_url(schema_name, checklist_id),
            version=settings.MANIFEST_VERSION.get(schema_name, ''),
            is_single_cell=True,
        )
        for checklist_id, value in rembi_checklists.items()
    ]
    rembi_items.sort(key=lambda x: x['label'])
    manifests.extend(rembi_items)

    # Spatial Transcriptomics FISH images
    schema_name = 'COPO_IMAGE_STX_FISH'
    stx_fish_checklists = singlecell_service.get_checklists(
        schema_name=schema_name, checklist_id=''
    )
    stx_fish_image_items = [
        build_manifest_item(
            value,
            manifest_id=checklist_id,
            manifest_type='stx_fish_images',
            label=value.get('name', ''),
            blank_manifest_url=get_blank_manifest_url(schema_name, checklist_id),
            version=settings.MANIFEST_VERSION.get(schema_name, ''),
            is_single_cell=True,
        )
        for checklist_id, value in stx_fish_checklists.items()
    ]
    stx_fish_image_items.sort(key=lambda x: x['label'])
    manifests.extend(stx_fish_image_items)

    # Single-cell
    schema_name = 'COPO_SINGLE_CELL'
    single_cell_checklists = singlecell_service.get_checklists(
        schema_name=schema_name, checklist_id=''
    )
    single_cell_items = [
        build_manifest_item(
            value,
            manifest_id=checklist_id,
            manifest_type='single_cell',
            label=value.get('name', ''),
            blank_manifest_url=get_blank_manifest_url(schema_name, checklist_id),
            version=settings.MANIFEST_VERSION.get(schema_name, ''),
            is_single_cell=True,
        )
        for checklist_id, value in single_cell_checklists.items()
    ]
    single_cell_items.sort(key=lambda x: x['label'])
    manifests.extend(single_cell_items)
