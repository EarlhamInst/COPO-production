...
class Email:
    ...
    def notify_sample_accepted_after_approval(self, **kwargs):
        ...
        if profile:
            ...
            apt_objs = AssociatedProfileType.objects.filter(
                name__in=apts, is_acceptance_email_notification_required=True
            )
            if not apt_objs:
                logger.debug(
                    f"No acceptance notificaion email required for the associated profile types {apts}. Skipping email notification."
                )
                return
            ...
            for sc in sequencing_centres:
                ...
                if (
                    settings.ENVIRONMENT_TYPE == "prod"
                    and centre.contact_details
                    and len(centre.contact_details) > 0
                ):
                ...
                centre_labels = d_utils.join_with_and(centre_labels)
            ...
            shared_profile_users_info = (
                CopoGroup().get_shared_users_info_by_owner_and_profile_id(
                    owner_id=profile_owner_userID, profile_id=profile.get('_id', '')
                )
            )
            shared_profile_users_emails = [
                element['email']
                for element in shared_profile_users_info
                if element.get('email', '')
            ]
            shared_profile_users_names = [
                element['name']
                for element in shared_profile_users_info
                if element.get('name', '')
            ]
            ...
            centre_contact_emails = [
                element['contact_email']
                for element in centre_contact_details
                if element.get('contact_email', '')
            ]
            ...
            if user:
                ...
                logger.log(
                    'No users found to send email to. Perhaps the user has been deleted.'
                )

    def notify_sample_rejected_after_approval(
        self, profile=None, rejected_sample=dict()
    ):
        ...
        sample_arr = [
            f"<li>{key} : {rejected_sample[key]}</li>" for key in rejected_sample.keys()
        ]
        ...
        if profile:
            ...
            # apts = [apt.get("value", "") for apt in associated_profile_types]
            apt_objs = AssociatedProfileType.objects.filter(
                name__in=apts, is_approval_required=True
            )
            for apt_obj in apt_objs:
        ...
        if users:
            ...
            msg = self.messages["sample_rejected"].format(
                profile.get("title", " "), profile.get("description", " "), sample_str
            )
            sub = (
    ...
    def notify_manifest_pending_for_associated_project_type_checker(
        ...
        if profile:
            ...
            if len(users) > 0:
                ...
                msg = self.messages["associated_project_samples"].format(
                    kwargs["title"],
                    demo_notification + kwargs["description"],
                    data,
                    data,
                )
                sub = demo_notification + " Manifest - " + kwargs["title"]
                CopoEmail().send(
                    to=list(email_addresses), sub=sub, content=msg, html=True
                )
                return
    ...
    def remind_manifest_pending_for_associated_project_type_checker(self):
        ...
        if results:
            ...
            apt_objs = AssociatedProfileType.objects.filter(
                name__in=associated_profile_types, is_approval_required=True
            )
            ...
            for apt_obj in apt_objs:
                ...
                if email_addresses:
                    ...
                    if records:
                        ...
                        for p in records:
                            profiles_by_type[p.get('type', '').upper()].append(
                                p.get('title', '')
                            )
                        ...
                        msg = (
                            self.messages['associated_project_samples_reminder']
                            .format(
                        ...
                        if associated_type in profile_types:
                            msg = msg.replace(
                                f' have {associated_type} as the associated profile type and',
                                '',
                            ).replace(f'<h3>{associated_type}</h3>', '')
                            sub = sub.replace(
                                f' with {associated_type} Association', ''
                            )
            ...
            logger.log(
                'No emails sent as no pending samples were found for any associated profile type sample manager'
            )
            return
