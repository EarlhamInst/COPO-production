...
class Command(BaseCommand):
    ...
    def check_for_default_news_image(self):
        ...
        default_news_image_source_path = os.path.join(
            settings.STATIC_ROOT, 'assets/img/news_image_default.jpg'
        )
        default_news_image_destination_path = os.path.join(
            news_images_directory, 'news_image_default.jpg'
        )
        ...
        if not os.path.exists(default_news_image_destination_path):
            ...
            shutil.copy2(
                default_news_image_source_path, default_news_image_destination_path
            )
        else:
    ...
    def handle(self, *args, **options):
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS('Removed existing news items and categories\n')
            )
            ...
            news_items = [
                ...
                {
                    'title': 'Disruption to ENA Submissions August 2024',
                    'content': (
                        '<p>Owing to a scheduled cluster downtime and maintenance at European Nucleotide Archive (ENA), all ENA submissions will be paused between Monday 19th August, 2024 and Thursday 22nd August, 2024.</p>'
                        '<p>Submissions will resume on Friday 23rd August, 2024.</p>'
                        '<p>Apologies for the disruption.</p>'
                        '<br><p>Refer to the <a href="https://copo-docs.readthedocs.io/en/latest/about/overview.html#where-your-submitted-data-goes" target="_blank">Where Your Submitted Data Goes</a> section for information about other public repositories that COPO supports.</p>'
                    ),
                    'category': NewsCategory.objects.get(name='Maintenance'),
                    'author': 'COPO Project Team',
                    'news_image': '/copo/static/assets/img/service_unavailable.jpg',
                },
                {
                    'title': 'COPO Marks First Successful Single-cell Submission',
                    'content': (
                        '<p>The Collaborative OPen Omics (COPO) platform facilitated its first single-cell data submission in January 2026 after development and testing of the new workflow was completed in 2025.</p>'
                        '<p>The submission was part of the project, <b>longread_singlecell_transcriptomics_approaches</b>, which involved long-read single-cell RNA sequencing technologies using matched with cDNA generated with the 10x Genomics platform. It used a Minimum Information about any (x) Sequence (MIxS) single-cell submission type and included metadata covering person, study, dissociation, cell suspension, library preparation, sequencing, raw data processing and data files. The submission employed the <a href="https://www.ebi.ac.uk/ena/browser/home" target="_blank">European Nucleotide Archive (ENA)</a> default sample checklist.</p>'
                        '<br><p>Highlights include:</p>'
                        '<ul><li>First successful use of single-cell submission workflow in COPO which demonstrates the platform\'s readiness for more single-cell metadata upload</li>'
                        '<li>Validation of long-read single-cell RNA sequencing metadata handling ensures that complex experimental metadata can be captured accurately</li>'
                        '<li>Integration with European Nucleotide Archive (ENA) workflows confirmed which includes the proper use of the various supported sample checklists types such as the default sample checklist</li>'
                        '<li>Uploaded samples are automatically prefilled in the single-cell spreadsheet with minimal sample metadata. This reduces time to complete the single-cell manifest.</li>'
                        '<li>Support for various types of single-cell submissions such as MIxS has been proven thereby, enabling standardised metadata capture</li>'
                        '<li>Enhanced traceability of samples and associated data from project, to metadata and through processed data files</li>'
                        '<li>Proof of concept for end-to-end submission automation which reduces manual data entry and potential errors</li>'
                        '<li>Foundation for future single-cell submissions which makes it easier for other users to use COPO submit single-cell data</li>'
                        '<li>Samples and single-cell metadata as well as the data files are publicly available (after the data is published) which allow others to cite them in publications or for reviewers to verify that reported results match the underlying metadata.</li>'
                        '</ul>'
                        '<br><p>To view this Single-cell submission, please see the following links:</p>'
                        '<ul><li><a href="https://www.ebi.ac.uk/ena/browser/view/PRJEB106558" target="_blank">ENA Project PRJEB106558</a></li>'
                        '<li><a href="https://www.ebi.ac.uk/ena/browser/api/xml/PRJEB106558?download=true" target="_blank">ENA Project PRJEB106558 XML</a>(XML will be downloaded)</li>'
                        '</ul>'
                        '<br><p>See also the <a href="https://singlecellschemas.org/" target="_blank">Single-cell website</a> and <a href="https://copo-docs.readthedocs.io/en/latest/submissions/biodata/single-cell-submissions.html" target="_blank">Single-cell documentation</a> for more information.</p>'
                    ),
                    'category': NewsCategory.objects.get(name='Feature'),
                    'author': 'COPO Project Team',
                    'news_image': '/copo/static/assets/img/single-cell.png',
                },
                {
                    'title': 'Exporting Data in JSON-LD Format',
                    'content': (
                        '<p>In addition to data exports in <a href="https://www.json.org/json-en.html" target="_blank">JSON</a>, <a href="https://www.researchobject.org/ro-crate/" target="_blank">Research Object Crate (RO-Crate)</a> and CSV formats, Collaborative OPen Omics (COPO) also supports data exports in <a href="https://json-ld.org/" target="_blank">JavaScript Object Notation for Linked Data (JSON-LD)</a>, which integrates schema definitions with metadata.</p>'
                        '<p>Currently, this feature is implemented for Single-cell submissions and can be accessed through the API endpoints below:</p>'
                        '<ul><li><a href="https://copo-project.org/static/swagger/apidocs_index.html#/Profile/get_profiles__profile_id__schema__schema_name__studies__study_id_" target="_blank">/profiles/{profile_id}/schema/{schema_name}/studies/{study_id} endpoint</a></li>'
                        '</ul>'
                        '<br><p>Refer to the <a href="https://copo-docs.readthedocs.io/en/latest/retrieval/copo-api.html" target="_blank">API documentation</a> for instructions on how to access these endpoints and the <a href="https://copo-docs.readthedocs.io/en/latest/submissions/biodata/single-cell-submissions.html" target="_blank">Single-cell submission documentation</a> for information about making submissions.</p>'
                    ),
                    'category': NewsCategory.objects.get(name='Feature'),
                    'author': 'COPO Project Team',
                    'news_image': '/copo/static/assets/img/json-ld.png',
                },
                {
                    'title': 'Snap, Upload, Submit, Publish: COPO Adds New Ways to Submit Image Data',
                    'content': (
                        '<p>As of May, 2026, there are a number ways that images can be submitted through the Collaborative OPen Omics (COPO) platform. There is additional metadata support for Spatial Transcriptomics Fluorescence In Situ Hybridisation (ST FISH) '
                        'and REcommended Metadata for Biological Images (REMBI), alongside the existing functionality of uploading sample-associated images.</p>'
                        '<p>The image submission feature allows persons in the scientific community to upload images and the associated metadata to the COPO then, submit and publish them to public repositories such as the <a href="https://www.ebi.ac.uk/ena/browser/home" target="_blank">European Nucleotide Archive (ENA)</a> '
                        'and <a href="https://zenodo.org/" target="_blank">Zenodo</a>. It streamlines the process of sharing image metadata thereby making it easier to disseminate findings and enhance the visibility of one\'s work.</p>'
                        '<br><p>For more information about how to submit images through COPO, please see the following documentation:</p>'
                        '<ul><li><a href="https://copo-docs.readthedocs.io/en/latest/submissions/images/rembi-image-submission.html" target="_blank">General image submissions (based on REMBI)</a></li>'
                        '<li><a href="https://copo-docs.readthedocs.io/en/latest/submissions/images/sample-image-submission-tol.html" target="_blank">Sample image submissions</a></li>'
                        '<li><a href="https://copo-docs.readthedocs.io/en/latest/submissions/images/st-fish-image-submission.html" target="_blank">Spatial Transcriptomics FISH image submissions</a></li>'
                        '</ul>'
                        '<br><p>Refer to the <a href="https://copo-docs.readthedocs.io/en/latest/about/overview.html#where-your-submitted-data-goes" target="_blank">Where Your Submitted Data Goes</a> section for information about other public repositories that COPO supports.</p>'
                    ),
                    'category': NewsCategory.objects.get(name='Feature'),
                    'author': 'COPO Project Team',
                    'news_image': '/copo/static/assets/img/cornfield2.jpg',
                },
                {
                    'title': 'Zenodo Metadata Submission Now Available',
                    'content': (
                        ...
                        '<li>Automatically generated citations are available on Zenodo in APA, Harvard, MLA, Vancouver, Chicago and IEEE formats</li>'
                        '<li>Metadata can be exported in formats such as JSON, JSON-LD and BibTeX</li>'
                        '<li>Submissions are versioned and this ensures that updates generate new, traceable versions.</li>'
                        '<li>Data submitted to Zenodo is preserved long-term and this facilitates open access dissemination to other systems</li>'
                        '<li>Improves reproducibility and data traceability - Sharing standardised metadata across research platforms is supported</li>'
                        '</ul>'
                        '<br><p>Refer to the <a href="https://copo-docs.readthedocs.io/en/latest/about/overview.html#where-your-submitted-data-goes" target="_blank">Where Your Submitted Data Goes</a> section for information about other public repositories that COPO supports and <a href="https://copo-docs.readthedocs.io/en/latest/submissions/biodata/single-cell-submissions.html" target="_blank">Single-cell submission documentation</a> for more information.</p>'
                    ),
                    'category': NewsCategory.objects.get(name='Feature'),
                    'author': 'COPO Project Team',
                    'news_image': '/copo/static/assets/img/zenodo.png',
                },
                {
                    'title': 'Disruption to ENA Submissions May 2026',
                    'content': (
                        '<p>Owing to a scheduled cluster downtime and maintenance at European Nucleotide Archive (ENA), all ENA submissions will be paused between Thursday 7th May, 2026 to Thursday 14th May, 2026.</p>'
                        '<p>Submissions will resume on Friday 15th May, 2026.</p>'
                        '<p>Apologies for the disruption.</p>'
                        '<br><p>Refer to the <a href="https://copo-docs.readthedocs.io/en/latest/about/overview.html#where-your-submitted-data-goes" target="_blank">Where Your Submitted Data Goes</a> section for information about other public repositories that COPO supports.</p>'
                    ),
                    'category': NewsCategory.objects.get(name='Maintenance'),
            ...
            for news_item in news_items:
                ...
                lg.debug(
                    f'Defining and creating the destination directory based on the news item ID'
                )
                # self.stdout.write(f'Defining and creating the destination directory based on the news item ID')

                destination_directory = os.path.join(
                    media_root, 'news_images', str(news_item.pk)
                )
                ...
                destination_path = os.path.join(
                    destination_directory, new_image_file_name
                )
            ...
            for x in news_records:
                self.stdout.write(
                    f'News item: {x.title}; Image URL: {x.news_image.url}'
                )
