...
from common.dal.mongo_util import (
    cursor_to_list,
    cursor_to_list_no_ids,
    cursor_to_list_str,
)
...
class Source(DAComponent):
    ...
    def get_specimen_biosample(self, value):
        return cursor_to_list(
            self.get_collection_handle().find(
                {
                    'sample_type': {
                        '$in': ['dtol_specimen', 'asg_specimen', 'erga_specimen']
                    },
                    'SPECIMEN_ID': value,
                }
            )
    ...
    def add_accession(
        self, biosample_accession, sra_accession, submission_accession, oid
    ):
    ...
    def get_sourcemap_by_specimens(self, value):
        sources = cursor_to_list(
            self.get_collection_handle().find({'SPECIMEN_ID': {'$in': value}})
        )
        source_map = {}
    ...
    def get_by_specimen_id_regex(self, value):
        ...
        return cursor_to_list(
            self.get_collection_handle().find(
                {'SPECIMEN_ID': {'$regex': value, '$options': 'i'}}
            )
        )
    ...
    def add_fields(self, fieldsdict, oid):
        return self.get_collection_handle().update_one(
            {'_id': ObjectId(oid)}, {'$set': fieldsdict}
        )

    def add_rejected_status(self, status, oid):
        return self.get_collection_handle().update_one(
            {'_id': ObjectId(oid)},
            {'$set': {'error': status['msg'], 'status': 'rejected'}},
        )
        ...
        return self.get_collection_handle().update_one(
            {'_id': ObjectId(oid)},
            {
                '$set': {
                    ...
                    'time_updated': datetime.now(timezone.utc).replace(microsecond=0),
                    'update_type': 'system',
                }
            },
        )
    ...
    def record_manual_update(self, field, old, new, oid):
        if not self.get_collection_handle().find(
            {'_id': ObjectId(oid), 'changelog': {'$exists': True}}
        ):
            self.get_collection_handle().update_one(
                {'_id': ObjectId(oid)}, {'$set': {'changelog': []}}
            )
        return self.get_collection_handle().update_one(
            {'_id': ObjectId(oid)},
            {
    ...
    def get_specimens_with_submitted_images(self, project, d_from, d_to):
        ...
        for specimen in specimens:
            ...
            specimen['tol_project'] = specimen['sample_type'].replace('_specimen', '')
...
class Sample(DAComponent):
    ...
    def get_sample_by_specimen_id_regex(self, specimen_id):
        ...
        return self.get_collection_handle().find(
            {'SPECIMEN_ID': {'$regex': specimen_id, '$options': 'i'}}
        )
            ...
            for cell in i:
                for field in sc:
                    ...
                    if cell == field.get('id', '').split('.')[-1] or cell == '_id':
                        if set(TOL_PROFILE_TYPES).intersection(
                            set(field.get('specifications', ''))
                        ):
        ...
        permit_filenames = [
            {
                ...
                if value.upper().replace(' ', '_').strip() != 'NOT_APPLICABLE'
            }
            for element in results
        ]

        # Remove duplicate permit file names from a list of dictionaries
        permit_filenames_unique = [
            element
            for index, element in enumerate(permit_filenames)
            if element not in permit_filenames[:index]
        ]

        # Get values from a list of unique dictionaries
        permit_filenames = [
            x for element in permit_filenames_unique for x in element.values()
        ]
        ...
        return self.get_collection_handle().count_documents(
            {
                ...
                'status': {'$nin': ['rejected', 'accepted', 'processing']},
            }
        )
            ...
            return self.get_collection_handle().update_many(
                {'_id': {'$in': records}},
                {
                    '$set': {
                        ...
                        + '.'
                        + index
                        + '.unit.annotationValue': element['value'],
                        char_or_fac
                        + '.'
                        + index
                        + '.unit.termSource': element['ontology_prefix'],
                        char_or_fac
                        + '.'
                        + index
                        + '.unit.termAccession': element['iri'],
                        char_or_fac
                        + '.'
                        + index
                        + '.unit.comments': element['description'],
                    }
                },
            )
        else:
                ...
                return self.get_collection_handle().update_many(
                    {'_id': {'$in': records}},
                    {
                        '$set': {
                            ...
                            + '.'
                            + index
                            + '.value.annotationValue': element['value']
                        }
                    },
                )
            else:
                ...
                return self.get_collection_handle().update_many(
                    {'_id': {'$in': records}},
                    {
                        '$set': {
                            ...
                            + '.'
                            + index
                            + '.value.termSource': element['ontology_prefix'],
                            char_or_fac
                            + '.'
                            + index
                            + '.value.termAccession': element['iri'],
                            char_or_fac
                            + '.'
                            + index
                            + '.value.comments': element['description'],
                        }
                    },
                )
    ...
    def save_record(self, auto_fields=dict(), **kwargs):
        ...
        for f in schema:
            ...
            if (
                f_specifications
                and profile_type not in f_specifications
                or f_manifest_version
                and current_schema_version not in f_manifest_version
            ):
                continue
    ...
    def delete_sample(self, sample_id):
        ...
        if sample.get('status', '') in [
            'accepted',
            'processing',
            'sending',
        ] or sample.get('biosmpleAccession', ''):
            ...
            if (
                ...
                message = message + 'Specimen with id {} was deleted'.format(
    ...
    def get_samples_by_taxon_id(self, taxon_ids):
        return cursor_to_list(
            self.get_collection_handle().find({'TAXON_ID': {'$in': taxon_ids}})
        )

    def get_all_dtol_samples(self):
        return cursor_to_list(
            self.get_collection_handle().find({'sample_type': 'dtol'}, {'_id': 1})
        )

    def get_project_samples(self, projects):
        return cursor_to_list(
            self.get_collection_handle().find(
                {'sample_type': {'$in': projects}}, {'_id': 1}
            )
        )
    ...
    def get_sample_accessions(self, element_dict):
        ...
        all_sample_types = [
            ...
            if sample_type not in EXCLUDED_SAMPLE_TYPES
        ]
        ...
        records = cursor_to_list_str(
            handler.find(filter, projection)
            .sort(sort_clause)
            .skip(int(start))
            .limit(int(length))
        )
        ...
        if records:
            ...
            for i in records:
                ...
                row_data['accession_type'] = (
                    GENOMICS_PROJECT_SAMPLE_TYPE_DICT.get(
                        i.get('sample_type', ''), ''
                    ).upper()
                    if i.get('sample_type', '') in GENOMICS_PROJECT_SAMPLE_TYPE_DICT
                    else i.get('tol_project', '').upper()
                )

                row_data.update(
                    {key: i.get(key, '') for key in i.keys() if key in labels}
                )
                out.append(row_data)
    ...
    def get_by_profile_id(self, profile_id):
        return cursor_to_list(
            self.get_collection_handle().find({'profile_id': profile_id})
        )
    ...
    def timestamp_dtol_sample_created(self, sample_id):
        ...
        sample = self.get_collection_handle().update_one(
            ...
            {
                '$set': {
                    'time_created': datetime.now(timezone.utc).replace(microsecond=0),
                    'created_by': email,
                }
            },
        )
    ...
    def timestamp_dtol_sample_updated(self, sample_id=str(), sample_ids=[]):
        ...
        self.get_collection_handle().update_many(
            {'_id': {'$in': sample_obj_ids}}, {'$set': set_update_data}
        )
    ...
    def update_field_by_query(self, query, field_values):
        ...
        try:
            email = ThreadLocal.get_current_user().email
            set_update_data['updated_by'] = email
            set_update_data['update_type'] = 'tempuser_' + str(
    ...
    def remove_field(self, field, oid):
        return self.get_collection_handle().update_one(
            {'_id': ObjectId(oid)}, {'$unset': {field: ''}}
        )

    def add_rejected_status(self, status, oid):
        return self.update_field(
            field_values={'error': status['msg'], 'status': 'rejected', 'approval': {}},
            oid=oid,
        )
    ...
    def get_by_profile_and_field(self, profile_id, field, value):
        return cursor_to_list(
            self.get_collection_handle().find(
                {field: {'$in': value}, 'profile_id': profile_id}, {'_id': 1}
            )
        )
    ...
    def get_by_project_and_field(self, project, field, value):
        return cursor_to_list(
            self.get_collection_handle().find(
                {field: {'$in': value}, 'tol_project': project}
            )
        )

    def get_dtol_from_profile_id(
        self,
        profile_id,
        filter,
        draw,
        ...
        if sort_by == '0':
            ...
            for field in sc:
                if (
                    not field.get('specifications', [])
                    or profile_type in field.get('specifications', [])
        ...
        if filter == 'pending':
            ...
            find_condition['status'] = {'$in': ['processing', 'sending']}

        else:
            find_condition['status'] = filter

        cursor = (
            handler.find(find_condition)
            .sort(sort_clause)
            .skip(int(start))
            .limit(int(length))
        )
        ...
        if samples:
            ...
            schema = [
                x
                for x in sc
                if x.get('show_in_table', True)
                and (
                    not x.get('specifications', [])
                    or profile_type in x.get('specifications', [])
                )
            ]
    ...
    def get_sample_display_column_names(self, group_filter=None):
        ...
        for field in sc:
            ...
            if (
                not field.get('specifications', '')
                or group_set.intersection(set(field.get('specifications', '')))
            ) and field.get('show_in_table', ''):
    ...
    def mark_rejected(
        self, sample_id=None, reason='Sample rejected by curator.', sample_ids=[]
    ):
        if not sample_ids:
            sample_ids = list()
        if sample_id:
            sample_ids.append(sample_id)
        return self.update_field(
            field_values={'status': 'rejected', 'error': reason, 'approval': {}},
            oids=sample_ids,
        )
        ...
        self.update_field(
            field_values={'status': 'rejected', 'error': reason}, oid=sample_id
        )
    ...
    def mark_pending(self, sample_ids, is_private=False):
        ...
        if is_private:
            ...
            status = 'pending'
        approval_again_associated_types = AssociatedProfileType.objects.filter(
            is_approval_required_for_updated_manifest=True
        )
        self.get_collection_handle().update_many(
            {'_id': {'$in': [ObjectId(oid) for oid in sample_ids]}},
            {
                '$unset': {
                    'approval.' + type.name: ''
                    for type in approval_again_associated_types
                },
    ...
    def get_pending_samples(self):
        ...
        profile_ids = self.get_collection_handle().distinct(
            'profile_id', {'status': 'pending', 'approval': {'$exists': True}}
        )
        ...
        if profile_ids:
            ...
            profile_data = cursor_to_list(
                profile_instance.get_collection_handle().find(
                    {'_id': {'$in': profile_ids_oid}},
                    {'_id': 0, 'title': 1, 'type': 1, 'associated_type': 1},
                )
            )
            ...
            for profile in profile_data:
                ...
                for associated_type in associated_types:
                    grouped_data[associated_type].append(
                        {'title': profile['title'], 'type': profile['type']}
                    )
    ...
    def get_by_manifest_id(self, manifest_id):
        ...
        samples = cursor_to_list(self.get_collection_handle().find(samples_filter))
        if samples:
            profile = cursor_to_list(
                handle_dict['profile'].find(
                    {'_id': ObjectId(samples[0]['profile_id'])}, {'title': 1}
                )
            )
            profile_title = ''
            if profile:
    ...
    def get_by_sequencing_centre(self, sequencing_centre, isQueryByManifestLevel=False):
        ...
        profile_ids_based_on_profile = cursor_to_list_no_ids(
            Profile().get_profile_by_sequencing_centre(
                sequencing_centre.name, getProfileIDOnly=True
            )
        )
        profile_ids_based_on_sample = self.get_sample_by_GAL(
            sequencing_centre.label, getProfileIDOnly=True
        )
        ...
        profile_ids_based_on_sample = [
            x.get('profile_id', str()) for x in profile_ids_based_on_sample
        ]

        # Get unique profile ids i.e. remove duplicates
        profile_ids = list(
            set(profile_ids_based_on_profile + profile_ids_based_on_sample)
        )
    ...
    def get_status_by_manifest_id(self, manifest_id):
        ...
        return cursor_to_list(
            self.get_collection_handle().find(
                status_filter,
                {'status': 1, 'manifest_id': 1, 'time_created': 1, 'time_updated': 1},
            )
        )
    ...
    def get_by_biosampleAccessions(self, biosampleAccessions):
        return cursor_to_list(
            self.get_collection_handle().find(
                {'biosampleAccession': {'$in': biosampleAccessions}}
            )
        )
    ...
    def get_manifests(self):
        cursor = self.get_collection_handle().aggregate(
            [
                {'$match': {'sample_type': {'$in': SANGER_TOL_PROFILE_TYPES}}},
                {'$sort': {'time_created': -1}},
                {
                    '$group': {
                        '_id': '$manifest_id',
    ...
    def get_manifests_by_date_and_project(self, projects, d_from, d_to):
        ids = self.get_collection_handle().aggregate(
            [
                {
                    '$match': {
    ...
    def add_blank_barcode_record(self, specimen_id, barcode_id):
        self.get_collection_handle().update_many(
            {'specimen_id': specimen_id},
            {'$set': {'specimen_id': specimen_id, 'barcode_id': barcode_id}},
            upsert=True,
        )
            ...
            self.get_collection_handle().update_one(
                {
                    ...
                    'read.file_id': {'$regex': id},
                    'read.$.status': {'$ne': status},
                },
        ...
        self.get_collection_handle().update_many(
            {
                ...
                'status': 'sending',
                'date_modified': {
                    '$lt': datetime.now(timezone.utc).replace(microsecond=0)
                    - timedelta(seconds=refresh_threshold)
                },
            },
            {'$set': update_data},
        )
    ...
    def get_custom_sample_fields(self):
        ...
        copo_defined_fields = [
            x['id'].split('.')[-1]
            for x in schema
            if not x.get('specifications', list())
            and not x.get('manifest_versions', list())
        ]
